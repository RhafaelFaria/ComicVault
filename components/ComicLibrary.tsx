...
export default function ComicLibrary({ onSelectComic }: ComicLibraryProps) {
  ...
  const [filterMode, setFilterMode] = useState<'all' | 'favorites' | 'unread'>('all'); // Novo estado de filtro
  ...
  // Filtros aplicados em cascata (primeiro abas, depois busca)
      ...
      {comics.length > 0 && (
        <div className="flex gap-2 mb-8 border-b border-gray-200 pb-2 overflow-x-auto">
          <button 
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors whitespace-nowrap ${filterMode === 'all' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:bg-gray-100'}`}
          >
            Toda a Coleção
          </button>
          <button 
            onClick={() => setFilterMode('favorites')}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors flex items-center gap-1.5 whitespace-nowrap ${filterMode === 'favorites' ? 'bg-rose-100 text-rose-700' : 'text-gray-500 hover:bg-gray-100'}`}
          >
            <Heart className="w-4 h-4" /> Favoritos
          </button>
          <button 
            onClick={() => setFilterMode('unread')}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors whitespace-nowrap ${filterMode === 'unread' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:bg-gray-100'}`}
          >
            Não Lidos
          </button>
        </div>
          ...
          {filteredComics.map((comic) => (
            ...
            >
              <div className="relative aspect-[2/3] w-full rounded-xl overflow-hidden shadow-md group-hover:shadow-xl transition-all duration-300 bg-gray-200">
                ...
                {/* Ícone de Lido e Favorito no topo da capa */}
                <div className="absolute top-2 left-2 flex flex-col gap-2">
                  {comic.isRead && (
                    <div className="bg-green-500 text-white p-1 rounded-full shadow-md" title="Lido">
                      <CheckCircle2 className="w-4 h-4" />
                    </div>
                  )}
                  <button 
                    onClick={(e) => handleToggleFavorite(e, comic.id)}
                    className={`p-1.5 rounded-full shadow-md transition-colors ${comic.isFavorite ? 'bg-rose-500 text-white' : 'bg-white/80 text-gray-400 hover:text-rose-500'}`}
                  >
                    <Heart className="w-4 h-4" fill={comic.isFavorite ? "currentColor" : "none"} />
                  </button>
                </div>

                {/* Barra de Progresso Visual na base da capa */}
                {comic.totalPages && comic.totalPages > 0 && !comic.isRead && (
                  <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-black/40">
                    <div 
                      className="h-full bg-indigo-500" 
                      style={{ width: `${((comic.currentPage || 0) / (comic.totalPages - 1)) * 100}%` }}
                    />
                  </div>
                )}
